...
const Hero = () => {
    ...
    return (
        <section className={`hero-section ${isDarkTheme ? 'dark-theme' : 'light-theme'}`} id="home">
            ...
            {/* A real <img>, not a CSS background — lets the browser discover and
                prioritize it during the initial page scan instead of only after
                CSS has parsed, which is what actually keeps LCP fast on a hero. */}
            <img
                className="hero-background"
                src={`${process.env.PUBLIC_URL}/Awards.jpg`}
                alt=""
                fetchpriority="high"
                decoding="async"
            />
            ...
            <div className="hero-content fade-in-up">
                ...
                <p className="hero-subtitle">
                    ChennaiLawyers.net is a full-service law firm based in Chennai, offering
                    practical legal solutions for individuals, businesses and institutions.
                </p>

                <div className="hero-cta-group">
                    <a href="#practice" className="apple-btn primary-btn">Our practice areas</a>
                    <a href="#contact" className="apple-btn secondary-btn">Get in touch</a>
                </div>
            </div>
        </section>
    );
...
/* ---------------------------------------------
   ...
   Design note: this section is a full-bleed editorial
   photo, so it keeps one dark scrim + white text in
   BOTH themes — that's what a photo hero needs to stay
   legible and look intentional rather than washed out.
   "Dual theme" here means the scrim's tint and the
   secondary button shift slightly to feel native to
   each theme, not that the photo goes light-on-light.
   That approach is what produced the foggy, low-contrast
   look in light mode — this fixes it at the source
   instead of patching opacity numbers.
--------------------------------------------- */
const CSS = `
.hero-section {
    --gold: #C49B55;
...
.hero-overlay {
    ...
    background: linear-gradient(
        ...
        rgba(var(--scrim), var(--scrim-strong)) 32%,
        rgba(var(--scrim), var(--scrim-mid)) 55%,
        rgba(var(--scrim), var(--scrim-soft)) 78%
    );
}
...
.fade-in-up {
    opacity: 0;
    animation: heroFadeUp 0.9s cubic-bezier(0.25, 0.8, 0.25, 1) forwards;
}
...
@media (max-width: 768px) {
    ...
    .hero-overlay {
        background: linear-gradient(
            to top,
            rgba(var(--scrim), var(--scrim-strong)) 25%,
            rgba(var(--scrim), var(--scrim-mid)) 100%
        );
    }
    .hero-cta-group {
        flex-direction: column;
